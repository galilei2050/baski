...
- **A tool's output must be bounded by the caller, not by the site** — `WebBrowseTool` takes a required `max_chars` (one measured fetch was 457,780 characters into a 32k-token agent). It does NOT cut blind: a long page returns one window plus the table of contents of the WHOLE page with each section's offset, which the agent passes back as `offset`. Cutting blind means neither side knows whether what mattered was in what was kept; this way nothing is hidden, only deferred. The tool is deliberately stateful — it holds the page behind the last fetch, so reading on costs no second browser load.
