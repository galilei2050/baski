...
- **A tool's output must be bounded by the caller, not by the site** — `WebBrowseTool` takes a required `max_chars` (one measured fetch was 457,780 characters into a 32k-token agent). It does NOT cut blind: a long page returns its opening plus the sections of the WHOLE page, and the agent asks for the ones it needs **by name** in `sections` — several at once, since paging costs a turn each and a turn re-reads the whole prefix. Cutting blind means neither side knows whether what mattered was in what was kept; this way nothing is hidden, only deferred. `offset` stays for a long page that lists no sections to name. The tool is deliberately stateful — it holds the pages it read, gzipped (2.9x measured; a month of browsing is 1.9 MB), so a follow-up section costs no second browser load.
